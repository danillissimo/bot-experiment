...
ItemUsageModule.Use['item_dust'] = function(item, bot, mode, extra_range)
	...
	if item:IsFullyCastable() == false then return BOT_ACTION_DESIRE_NONE end
	...
	for i = 1, #enemyPids do
		...
		if IsHeroAlive(enemyPids[i]) == true and info ~= nil then
			...
			then	
				...
				if IsLocationVisible(dInfo.location) == true then
					bot:ActionImmediate_Chat('Dust: lost sight of ' .. GetSelectedHeroName(enemyPids[i]), true);
...
local tango_cast_range = 165;
local tango_regen = 7.0;
local tango_duration = 16;
local tango_heal = tango_regen * tango_duration;

function CalcBasicHealHpDesire(bot)
	return RemapValClamped(
		bot:GetHealth()/bot:GetMaxHealth(),
		0.15,						0.75,
		BOT_ACTION_DESIRE_VERYHIGH, BOT_ACTION_DESIRE_NONE
	);
end
...
ItemUsageModule.Use['item_tango'] = function(item, bot, mode, extra_range)
	...
	local allies = bot:GetNearbyHeroes(sim.NIGHT_VISION_RADIUS, false, BOT_MODE_NONE)
	local max_desire = 0;
	local most_suffering_ally = nil;

	for _, ally in pairs(allies) do
		local tangoSlot = ally:FindItemSlot('item_tango');
		local singleTangoSlot = ally:FindItemSlot('item_tango_single');
		if 
			ally ~= bot
			and not ally:IsIllusion() 
			and tangoSlot > -1
			and singleTangoSlot > -1
			and ally:HasModifier("modifier_rune_regen") == false
			and ally:HasModifier("modifier_filler_heal") == false
			and ally:DistanceFromFountain() > sim.NIGHT_VISION_RADIUS
		then
			local distance = GetUnitToUnitDistance(bot,ally);
			local desire = RemapValClamped(
				distance,
				sim.BASIC_VISION_RADIUS * 5/4, tango_cast_range + extra_range,
				BOT_ACTION_DESIRE_NONE, 	   CalcBasicHealHpDesire(ally)
			);
			if desire > max_desire then
				max_desire = desire;
				most_suffering_ally = ally;
			end
		end
	end
	

	local self_desire, tree = ItemUsageModule.Use['item_tango_single'](item, bot, mode, extra_range);
	if 
		self_desire > BOT_ACTION_DESIRE_NONE
		or max_desire > BOT_ACTION_DESIRE_NONE
	then
		if self_desire > max_desire then
			return self_desire, tree, 'tree';
		else
			return max_desire, ally, 'unit';
		end
	else
		return BOT_ACTION_DESIRE_NONE;
	end
...
ItemUsageModule.Use['item_tango_single'] = function(item, bot, mode, extra_range)
	...
	local desire = CalcBasicHealHpDesire(bot);
	...
	if #enemies > 0 then
		-- Danger, tehe mpre hp, the wider search range
		local max_distance = RemapValClamped(
			bot:GetHealth() / bot:GetMaxHealth(),
			0.15,								0.75,
			0, sim.NIGHT_VISION_RADIUS * 3/4 - cast_range - extra_range
		);
		trees = bot:GetNearbyTrees(cast_range + extra_range + max_distance);
		if #trees > 0 then
			for i=1, #trees do
		...
		-- Better nothing then this at current state
		--
...
ItemUsageModule.Use['item_flask'] = function(item, bot, mode, extra_range)
	...
	for _,ally in pairs(allies) do
		if ally:IsIllusion() == false then
			...
			local desire = CalcBasicHealHpDesire(ally);
